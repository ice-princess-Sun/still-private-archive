...
- 新发布内容默认排在首页最前。
- 旧内容会按原发布时间初始化排序值，避免影响既有图文内容。
- 每次调整顺序后自动重新编号，降低排序值冲突风险。

### 对应提交

- `Add manual homepage ordering`

## [0.8.0] - 2026-06-26

### 新增

- 新增详情页自适应图片相框，自动区分横图、竖图和方图。

### 优化

- 详情页图片改为完整展示，避免竖屏照片被横向相框强制裁切。
- 首页封面和管理后台缩略图减少裁切，更适合竖屏照片预览。

### 对应提交

- `Improve portrait photo support`

## [0.7.0] - 2026-06-26

### 新增

- 增加根据访问者本地时间自动变化的网站氛围。
- 首页顶部提示会随清晨、白天、黄昏和夜晚显示不同文案。

### 优化

- 清晨、白天、黄昏和夜晚分别使用轻微不同的纸张色、文字色和背景光感。
- 时间氛围在浏览器端判断，不依赖 Render 服务器时区。
- 氛围变化保持克制，不影响原有登录保护和内容权限。

### 对应提交

- `Add time based ambience`
